...
def load_state(fname,step, saver=None):
    """Load all the variables to the current session from the location <fname>"""
    if saver is None:
        saver = tf.train.Saver()
    saver.restore(get_session(), fname+str(step))
    return saver
...
def save_state(fname,step, saver=None):
    ...
    if saver is None:
        saver = tf.train.Saver()
    saver.save(get_session(), fname+str(step))#max_to_keep=4,
    return saver
...
def function(inputs, outputs, updates=None, givens=None,type=None):
...
class _Function(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        for inpt, value in zip(self.inputs, *args):
            self._feed_input(feed_dict, inpt, [value[0].tolist()])#---
        # Update the kwargs
