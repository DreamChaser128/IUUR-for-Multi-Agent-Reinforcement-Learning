...
def p_train(make_obs_ph_n, act_space_n,  p_index,p_func, q_func, optimizer, grad_norm_clipping=None, num_units=64, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        p_input = obs_ph_n[:len(obs_ph_n)-1]
        ...
        for i in range(len(c)):
            ...
            p_reg_list+=tf.reduce_mean(tf.square(act_pd.flatparam()))
        ...
        act = U.function(inputs=obs_ph_n[:len(obs_ph_n)-1], outputs=act_sample1)
        p_values = U.function(obs_ph_n[:len(obs_ph_n)-1], p)

        # target network
        target_p = p_func(p_input, int(act_pdtype_n[p_index].param_shape()[0]), scope="target_p_func", num_units=num_units)
        target_f = tf.reshape(target_p, [-1, 50])
        c_target = tf.split(target_f, 10, 1)
        ...
        target_act = U_p.function(inputs=obs_ph_n[:len(obs_ph_n)-1], outputs=act_sample2)
...
class MADDPGAgent_public_network():
    def __init__(self,  model,obs_shape_n, act_space_n, args, local_q_func=False):
        obs_ph_n_q=[]
        obs_ph_n_p=[]
        ...
        obs_ph_n_q.append(U.BatchInput((51,), name="observation" + str(self.n)).get())
        for i in range(self.n):
            obs_ph_n_p.append(U.BatchInput(obs_shape_n[i], name="observation"+str(i)).get())
        obs_ph_n_p.append(U.BatchInput((51,), name="observation" + str(self.n)).get())
...
class MADDPGAgentTrainer(AgentTrainer):
    ...
    def update(self, agents,p_index,public_network,t):
        if len(self.replay_buffer) < self.max_replay_buffer_len: # replay buffer is not large enough 5000
            return
        ...
        if self.agent_index ==p_index:
            target_act_next_n = public_network.p_debug['target_act'](obs_next_n)
            target_act_next_nn=target_act_next_n[0]
            obs_next_nc = obs_next_n.copy()
            obs_next_nc.append(obs_next)
            target_q_next = public_network.q_debug['target_q_values'](*(obs_next_nc + target_act_next_nn))
        ...
        #update network
        public_network.p_update()
        public_network.q_update()
