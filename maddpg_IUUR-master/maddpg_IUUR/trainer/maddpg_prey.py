...
def p_train(make_obs_ph_n, act_space_n,  p_index,p_func, q_func, optimizer, grad_norm_clipping=None,num_units=64, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        p_input = obs_ph_n[p_index:len(obs_ph_n)-1]
        ...
        act = U.function(inputs=obs_ph_n[p_index:len(obs_ph_n)-1], outputs=act_sample1)
        p_values = U.function(obs_ph_n[p_index:len(obs_ph_n)-1], p)

        # target network
        target_p = p_func(p_input, int(act_pdtype_n[p_index].param_shape()[0]), scope="target_p_func", num_units=num_units)
        target_f = tf.reshape(target_p, [-1, 10])
        c_target = tf.split(target_f, 2, 1)
        ...
        target_act = U_p.function(inputs=obs_ph_n[p_index:len(obs_ph_n)-1], outputs=act_sample2)
...
class MADDPGAgent_public_network_prey():
    def __init__(self,  model,obs_shape_n, act_space_n, args):
        ...
        obs_ph_n_q.append(U.BatchInput((24,), name="observation" + str(self.n)).get())
        for i in range(self.n):
            obs_ph_n_p.append(U.BatchInput(obs_shape_n[i], name="observation"+str(i)).get())
        obs_ph_n_p.append(U.BatchInput((24,), name="observation" + str(self.n)).get())
    ...
    def action(self, obs,p_index):
        ...
        for i in range(len(obs)):
            obss.append([obs[i]])
        acts = self.act(obss[6:len(obss)])
        for i in range(len(acts)):
            acts1.append(acts[i][0])
        return acts1

class MADDPGAgentTrainer_prey(AgentTrainer):
    ...
    def update(self, agents,p_index,public_network_prey,public_network_good,t):
        ...
        if len(self.replay_buffer) < self.max_replay_buffer_len: # replay buffer is not large enough 8000
            return
        ...
        if self.agent_index ==p_index:
            ...
            target_q_next = public_network_good.q_debug['target_q_values'](*(obs_next_nc + target_act_next_ns))
